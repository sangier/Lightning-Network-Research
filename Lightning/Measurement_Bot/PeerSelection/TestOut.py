...
print(str(dataNet['nodes'][3]['pub_key'])+"@"+str(dataNet['nodes'][3]['addresses'][0]["addr"]))

if(dataNet['nodes'][1]['addresses']==[]):
    print(dataNet['nodes'][1]['addresses'])
print(dataNet['nodes'][1]['addresses'])
# n=0
# p=0
# for m in range(0,len(data['channels'])):
    # for c in range(0,len(dataNet["nodes"])):
        # if (data['channels'][n]["Node1_pub_key"]=="0217890e3aad8d35bc054f43acc00084b25229ecff0ab68debd82883ad65ee8266" and data['channels'][n]["Channel_direction"]==0 and data['channels'][n]["EstimatedCapacity"]==10):
            # if (data['channels'][n]["Node2_pub_key"]==dataNet['nodes'][p]['pub_key']):
                # if(dataNet['nodes'][p]['address']!=None):
                    # NewPeers.append({  
                    # 'Address': dataNet['channels'][p]['pub_key'],

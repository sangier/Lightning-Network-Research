...
#Step 1: Connection to target peer
    ...
    address_only_pub=ad
    address=ad_ip
    cmd="lncli connect "+address
    print(payment_hash)
    cmd_net="lncli describegraph >mainnet_graph.json"
...
def Channel_Establishment():
    ...
    value=4500000 
    #value=20000		   
    cmd2="lncli openchannel "+address_only_pub+" "+str(value)
...
def Query_Select_Routes(LambdaT):
    ...
    global def_my_string
    testFail=0
    cmd3="lncli queryroutes "+choosed_peer+" "+str(LambdaT)+" --final_cltv_delta=144"
...
def EstimateCapacity():
    ...
    if(Query_Select_Routes(L)==1):
        if(Query_Select_Routes(epsilon)==1):
            ...
            r1=EnoughCapacity(epsilon)
            if(r1==2):
                return(-2,0)
            if(r1==1):
                end1=time.time()
                print("Channel measured in s:  "+str(end1-start1))
        ...
        if(r1==2):
            return(-2,0)
        if(r1==0):
            r1=EnoughCapacity(L+epsilon)
            if(r1==2):
                return(-2,0)
            if(r1==0):
                r1=EnoughCapacity(L+epsilon2)
                if(r1==2):
                    return(-2,0)
                if(r1==0):
                    end1=time.time()
                    print("Tempo di esecuzione per la misura di un canale: "+str(end1-start1))
                    return(L+epsilon2,(end1-start1))
                else:
                    end1=time.time()
                    print("Channel measured in s: "+str(end1-start1))            
                ...
                end1=time.time()
                print("Channel misured in s: "+str(end1-start1))            
                return(L,(end1-start1))
        r1=EnoughCapacity(epsilon)
        if(r1==2):
            return(-2,0)
...
def Main():
    ...
    j=0
    for i in range(0,len(peerList[0])):
        channelExist=0
        t2=0
        t=Connection_To_Peer(str(peerList[0][j]),str(peerList_ip[0][j]))
        if(t==2):
            cmd2_1="lncli listchannels"
            log2_1= subprocess.Popen([cmd2_1],stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
            output,error=log2_1.communicate()
            ...
            if(channelExist!=1):
                ...
                if(t2==1):
                    ...
                    if(m==1):
                        ...
                        print("Peer "+str(peerList[i])+" Measurement successfully ended in :"+str(start-end)+" seconds!!")
                        m=0
        elif(t==1):
            t2=Channel_Establishment()		
            if(t2==1):

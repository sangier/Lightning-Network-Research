...
#Step 1: Connection to target peer
    ...
    global ConnectO	
    p=0
    address_only_pub="02a68237add204623021d09b0334c4992c132eb3c9dcfcb8f3cf8a57386775538e"
    address="02a68237add204623021d09b0334c4992c132eb3c9dcfcb8f3cf8a57386775538e@18.205.95.237:9735"
...
def Close_Channel():
    ...
    ok=0
    while(ok!=1):
        #if(closing["waiting_close_channel"][0]!=None or closing["pending_force_closing_channels"][0]!=None or closing["pending_closing_channels"][0]!=None):
        if(closing["waiting_close_channel"][0]!=None or closing["pending_closing_channels"][0]!=None): #HERE YOU HAVE TO ADD THE FORCE PENDING CONTROL
            print("We have to wait 12 minutes to check again the channel closing")
            time.sleep(730)
        else:
            print("Channel with reference "+str(TxID)+"successfully closed")			
            ok=1
...
#second hoP: 03ecd95e86e780ae82139c3217b527073622d28c4d8a53b76142a7b7b1d1e36975  Previous: 02fd753c8ad77d2601637b6add4362fec59f4d3e8190347ab4d155f3e9a76e113c
    ...
    cmd4='lncli --network=testnet sendtoroute --payment_hash=35ac914e2b9d4e30de1a12637fe49d3125ec0560b2b9af5d1b4cdf3884e87512 --routes="$(cat route1.json)" -'
...
def EstimateCapacity():
    ...
    if(Query_Select_Routes(L)==1):
        ...
        if(EnoughCapacity(L)==0):
            if(EnoughCapacity(L+epsilon)==0):
                end1=time.time()
                print("Tempo di esecuzione per la misura di un canale: "+str(end1-start1))
                return(L+epsilon,(end1-start1))
            else:
                end1=time.time()
                print("Tempo di esecuzione per la misura di un canale: "+str(end1-start1))            

...
def Channel_Establishment():
    ...
    with open('Channel_Establishment_Log.txt', 'r+') as f2:
        ...
        if now2.startswith("[lncli]"):
            ...
            with open('Channel_Establishment_Log.txt', 'r+') as f3:
                creation=json.loads(f3.read())
                time.sleep(10)
            TxID=creation["funding_txid"]
            print("The Funding Transaction is: "+str(TxID))
            cmd2_1="lncli -network=testnet listchannels"
            with open('List_Channels_Log.txt', 'w+') as log2_1:
                c = subprocess.Popen([cmd2_1], stdout=log2_1, stderr=log2_1, shell=True)
                time.sleep(10)
            ok=0
            while(ok!=1):
                with open('List_Channels_Log.txt', 'r+') as f4:
                    list=json.loads(f4.read())
                j=0
...
def Close_Channel():
    ...
    with open('Channel_close_Log.txt', 'w+') as clog1:
        c = subprocess.Popen([cmd2], stdout=clog1, stderr=clog1, shell=True)
        time.sleep(30)		
    
    cmdc="lncli -network=testnet pendingchannels"
    with open('Channel_pending_Log.txt', 'w+') as clog2:
        c = subprocess.Popen([cmdc], stdout=clog2, stderr=clog2, shell=True)
        time.sleep(30)		
    	
    with open('Channel_pending_Log.txt', 'r+') as c2:
        closing=json.loads(c2.read())
        time.sleep(10)

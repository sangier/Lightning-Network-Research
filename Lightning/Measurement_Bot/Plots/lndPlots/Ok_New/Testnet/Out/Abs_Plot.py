...
for i in df["channels"]:
    if(int(df["channels"][k]["EstimatedCapacity"])!=-1 and int(df["channels"][k]["EstimatedCapacity"])!=-2 and int(df["channels"][k]["EstimatedCapacity"])!=-10 and int(df["channels"][k]["Channel_direction"]==0)): 
...
ax.set_title("Out Testnet")

fig = ax.get_figure()
fig.savefig('Out_Testnet.png',bbox_inches='tight')

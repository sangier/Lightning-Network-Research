...
for i in df["channels"]:
    if(int(df["channels"][k]["EstimatedCapacity"])!=-1 and int(df["channels"][k]["EstimatedCapacity"])!=-2 and int(df["channels"][k]["EstimatedCapacity"])!=-10): 
...
ax.set_title("In_Out Testnet")

fig = ax.get_figure()
fig.savefig('In_Out_Testnet.png',bbox_inches='tight')

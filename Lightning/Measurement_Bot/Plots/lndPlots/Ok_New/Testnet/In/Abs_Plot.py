...
df = pd.read_json("TestnetData.json")
print(df)
...
for i in df["channels"]:
    if(int(df["channels"][k]["EstimatedCapacity"])!=-1 and int(df["channels"][k]["EstimatedCapacity"])!=-2 and int(df["channels"][k]["EstimatedCapacity"])!=-10 and int(df["channels"][k]["Channel_direction"]==1)): 
...
ax.set_title("In Channels Testnet")

fig = ax.get_figure()
fig.savefig('In_Testnet.png',bbox_inches='tight')
